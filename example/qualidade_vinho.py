...
arqvinho = "./dados/qualidade_vinho.csv"
...
neurons = [entrada.shape[1], 15, 6, 4,  4]

rna = RNAE(neurons)

rna.treinamento(entrada[0:n_trei, :], saida[0:n_trei, :],
             tpop=500, gens=1500, bits=16, pmut=0.1, pcruz=0.6, epidemia=150,
              xr=[-10, 20])

rna.salvar_modelo("./modelos/rnae_vinho.txt")
rna = RNAE.carregar_modelo("./modelos/rnae_vinho.txt")
